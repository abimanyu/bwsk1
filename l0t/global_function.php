...
function createSlugDB($str, $sql_table="") {
    if(!empty($sql_table)){
        global $sql;
        $createSlug = createSlug($str);
        $sql -> db_Select($sql_table, "slug", "WHERE `slug` LIKE '".$createSlug."%'");
        if($sql -> db_Rows()>=1) {
            $jum = $sql->db_Rows() + 1;
            return createSlugDB( createSlug ($str."-".$jum) , $sql_table);
        }else {return $createSlug;}
    } 
}
...
function _ago($tm,$rcs = 0) {
    ...
    $cur_tm = time(); 
    $dif = $cur_tm-$tm;
    $pds = array('dtk','menit','jam','hari','minggu','bulan','tahun');
